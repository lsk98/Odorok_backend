...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResponseRoot<Void>> handleUpload(FileUploadException e) {
        log.error("파일 업로드 중 문제발생 - {}", e.getMessage(),e);
        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
                .body(CommonResponseBuilder.fail("파일 업로드 실패: "));
    }
    ...
    public ResponseEntity<ResponseRoot<Void>> handleUnauthorizedException(UnauthorizedException e) {
        log.error("Unauthorized error - {}", e.getMessage(), e);
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(CommonResponseBuilder.fail("로그인 정보가 일치하지 않습니다"));
    }
    ...
    public ResponseEntity<ResponseRoot<Void>> handleDataIntegrityViolation(DataIntegrityViolationException e) {
        ...
        return ResponseEntity
                .status(HttpStatus.BAD_REQUEST)
                .body(CommonResponseBuilder.fail("글 작성에 실패했습니다."));
    }
